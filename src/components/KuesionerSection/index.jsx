...
import { createKuesioner } from "../../services/kuesionerService";
          ...
          question.name === "skala_usaha" ? (
            <div key={index} className="flex flex-col w-full">
              <CustomSelect
                ...
                isDisabled={question.name === "rt_rw_dusun" ? true : false}
              />
...
const KuesionerSection = ({ initialData }) => {
    ...
    if (initialData) {
      setTimeout(() => {
        setFormValues((prevValues) => ({
          ...prevValues,
          rt_rw_dusun: initialData.rt_rw_dusun,
          no_urut_bangunan: initialData.no_urut_bangunan,
          alamat: initialData.alamat,
          nama_pemilik_penanggungjawab: initialData.nama_kk_ak_bangunan,
  ...
  const canNavigate = useMemo(() => {
    ...
    for (let i = 0; i < currentBlockQuestions.length; i++) {
      const question = currentBlockQuestions[i];
      if (
        !formValues[question.name] ||
        (errors[question.name] && errors[question.name].length > 0)
      ) {
        return false;
      }
    }
  ...
  const createProcessing = async () => {
    setIsLoading(true);
    try {
      const data = await createKuesioner(formValues);
      console.log("Data:", data);
      message.success("Data kuesioner berhasil ditambahkan", 5);
      navigate("/kuesioner");
    } catch (error) {
      console.log("Error:", error);
      message.error("Data kuesioner gagal ditambahkan", 5);
    } finally { 
      setIsLoading(false);
    }
  };
  ...
  const handleCloseModal = () => {
    setIsSimpanModalOpen(false); // Close the modal
  };
  ...
  return (
    <div className="flex flex-col items-center w-full gap-5 mb-10">
      ...
      <h1 className="text-xl font-bold text-center">
        Kuesioner Pendataan UMKM Desa Simoangin-angin 2024
      </h1>
...
export default KuesionerSection;
